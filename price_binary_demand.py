...
def p6(price_historical,demand_historical,parameterdump=None):
    ...
    #day 3 to day 10
    if demand_historical.size>1 and demand_historical.size<100:
    ...
    if demand_historical.size>100:
        ...
        while judge==False:
            ...
            parameter_new=[]  
            for k in range(K):
                parameter_new.append(minimize(MLE,parameter[k],args=(price_mx,demand,possible_D[k],wl[k]),bounds=((-5,None),(0.001,10)),method='L-BFGS-B').x)
            theta_new=[np.mean(wl[j]) for j in range(K)]
